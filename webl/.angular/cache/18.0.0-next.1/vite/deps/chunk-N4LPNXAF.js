...
var _ensureDirtyViewsAreAlwaysReachable = true;
...
function markViewDirty(lView) {
  const dirtyBitsToUse = isRefreshingViews() ? 64 : (
...
function getCallbackScheduler() {
  ...
  if (typeof Zone !== "undefined") {
    ...
    if (nativeRequestAnimationFrame) {
      nativeRequestAnimationFrame = nativeRequestAnimationFrame[ORIGINAL_DELEGATE_SYMBOL] ?? nativeRequestAnimationFrame;
    }
...
function setRootNodeAttributes(hostRenderer, componentDef, hostRNode, rootSelectorOrNode) {
  if (rootSelectorOrNode) {
    setUpAttributes(hostRenderer, hostRNode, ["ng-version", "18.0.0-next.2"]);
  } else {
...
function ɵɵprojection(nodeIndex, selectorIndex = 0, attrs, fallbackTemplateFn, fallbackDecls, fallbackVars) {
  ...
  if (isEmpty2 && fallbackTemplateFn) {
...
function insertFallbackContent(lView, tView, projectionIndex, templateFn, decls, vars, attrs) {
  ...
  const fallbackLView = createAndRenderEmbeddedLView(lView, fallbackTNode, void 0, { dehydratedView });
...
var VERSION = new Version("18.0.0-next.2");
...
function detectChangesInView(lView, notifyErrorHandler, isFirstPass) {
  const mode = isFirstPass || lView[FLAGS] & 64 ? 0 : 1;
      ...
      next: () => {
        this.zone.run(() => {
          if (this.changeDetectionScheduler) {
            this.changeDetectionScheduler.tick(
              true
              /* shouldRefreshViews */
            );
          } else {
...
function internalProvideZoneChangeDetection({ ngZoneFactory, schedulingMode }) {
  return [
    ...
    schedulingMode === 1 ? { provide: ZONELESS_SCHEDULER_DISABLED, useValue: true } : [],
    // Only provide scheduler when explicitly enabled
    schedulingMode === 0 ? { provide: ChangeDetectionScheduler, useExisting: ChangeDetectionSchedulerImpl } : []
  ];
}
...
@angular/core/fesm2022/core.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-N4LPNXAF.js.map
